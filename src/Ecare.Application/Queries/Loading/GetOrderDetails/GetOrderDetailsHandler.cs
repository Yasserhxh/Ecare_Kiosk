...
public sealed class GetOrderDetailsHandler : IRequestHandler<GetOrderDetailsQuery, Result<ScanBySlvLoading>>
{
    ...
    private readonly ILogger<GetOrderDetailsHandler> _log;
    ...
    public GetOrderDetailsHandler(
        ...
        ILogger<GetOrderDetailsHandler> log)
    {
        _drivers = drivers;
        _orders = orders;
    ...
    public async Task<Result<ScanBySlvLoading>> Handle(GetOrderDetailsQuery request, CancellationToken ct)
    {
        ...
        try
        {
            ...
            if (equipement is null)
                return Result<ScanBySlvLoading>.Fail("Carte SLV inconnue/inactive");
            ...
            var result = new ScanBySlvLoading(
                firstWeight,
                equipement.Id,
            ...
            await SignalRHelper.BroadcastAsync(
                _signalR,
                hubName: "loading_data_hub",
                methodName: "LoadingDataEvent",
                payload: payload,
                logger: _log,
                ct: ct
            );
            ...
            return Result<ScanBySlvLoading>.Ok(result);
        }
        catch (Exception ex)
        {
