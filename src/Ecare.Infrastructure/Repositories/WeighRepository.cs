...
public sealed class WeighRepository : IWeighRepository
{
    ...
    {
        w.Id = Guid.NewGuid();
        await uow.Connection.ExecuteAsync(@"INSERT INTO Weighings (Id,OrderId,Stage,GrossKg,TareKg,NetKg,TakenAt)
    ...
    {
        var rows = await uow.Connection.QueryAsync<WeighRecord>("SELECT * FROM Weighings WHERE OrderId=@orderId",
